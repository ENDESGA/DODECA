...
#include <SDL2/SDL.h> // or wherever your SDL2 library is

#include "DODECA_CORE.h"
using namespace DODECA; // set the namespace
...
int main(int, char* [])
{
	...
	while (!QUIT)
	{
		...
		auto diff = std::chrono::duration_cast <std::chrono::nanoseconds> (std::chrono::system_clock::now() - start).count();
